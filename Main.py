...
import numpy as np 
import random
from Features.Open import OpenExe,CloseExe
from Whatsapp import WhatsappSender
from sendmail import get_email_info
from wifi_demo import LED_ON,LED_OFF
from translator import translator1
from TV_automation import TV
from Features.Wakeup import WakeupDetected

from basic_task import Time,Date,Day,auto,Input_func,NonInput_func
...
def MainTaskExecution(Query):
    ...
    try:
        ...
        elif "new tab" in ReturnData:
          auto(TaskNew)
         
        elif "new window" in ReturnData:
         auto(TaskNew)
         
        elif "history" in ReturnData:
         auto(TaskNew) 
         
        elif "add to bookmark" in ReturnData:
         auto(ReturnData)
         
        elif "download folder" in ReturnData:
         auto(ReturnData)
         
        elif "google search" in ReturnData:
           Input_func(ReturnData)
           
        elif "youtube search" in ReturnData:
           Input_func(ReturnData)    
        
        elif "shutdown" in ReturnData:
         NonInput_func(ReturnData)
         
        elif "restart" in ReturnData:
         NonInput_func(ReturnData) 
        ...
        elif "turn on" in ReturnData:
          LED_ON()
          
        elif "turn off" in ReturnData:
          LED_OFF()
          
        elif "translator" in ReturnData:
           translator1()
